...
import {WorkshopStackProps} from "./props";
...
export class WorkshopStack extends Stack {
  ...
  readonly namespace: string = "workshop.local";
  ...
  state: WorkshopStackProps;

  constructor(scope: Construct, id: string, props?: StackProps) {
    super(scope, id, props);

    if (!(props instanceof WorkshopStackProps)) {
      throw new Error("props must be an instance of StackPropsExt");
    }
  ...
  createLogGroup() {
    this.state.logGroup = new LogGroup(this, "LogGroup", {
  ...
  createVpc() {
    ...
    this.state.vpc = new Vpc(this, "VPC", {
      cidr: "10.0.0.0/16",
      maxAzs: 2,
      subnetConfiguration: [
        {
    ...
    this.state.externalSecurityGroup = new SecurityGroup(this, "ExternalSG", {
      vpc: this.state.vpc,
      allowAllOutbound: true,
    });
    this.state.externalSecurityGroup.connections.allowFromAnyIpv4(Port.tcp(80));
    ...
    this.state.internalSecurityGroup = new SecurityGroup(this, "InternalSG", {
      vpc: this.state.vpc,
      allowAllOutbound: true,
    });
    [Port.tcp(this.APP_PORT), Port.tcp(9901), Port.tcp(15000)].forEach(port => {
      this.state.internalSecurityGroup.connections.allowInternally(port);
    });
  }

  createCluster() {
    // Deploy a Fargate cluster on ECS
    this.state.cluster = new Cluster(this, "Cluster", {
      vpc: this.state.vpc,
    });
    ...
    let ns = this.state.cluster.addDefaultCloudMapNamespace({
      name: this.namespace,
    });
    // we need to ensure the service record is created for this after we enable app mesh
    // (there is no resource we create here that will make this happen implicitly
    // since CDK won't allow two services to register the same service name in
    ...
    */
    ...
    this.state.taskRole = new Role(this, "TaskRole", {
    ...
    this.state.taskExecutionRole = new Role(this, "TaskExecutionRole", {
      assumedBy: new ServicePrincipal("ecs-tasks.amazonaws.com"),
      managedPolicies: [
  ...
  createGateway() {
    let gatewayTaskDef = new FargateTaskDefinition(this, "GatewayTaskDef", {
      family: "gateway",
      taskRole: this.state.taskRole,
      executionRole: this.state.taskExecutionRole,
      cpu: 512,
      memoryLimitMiB: 1024,
    });
    ...
    let gatewayService = new FargateService(this, "GatewayService", {
      ...
      desiredCount: 1,
      securityGroup: this.state.internalSecurityGroup,
      cloudMapOptions: {
        name: "gateway",
      },
    });
    ...
    let alb = new ApplicationLoadBalancer(this, "PublicALB", {
      vpc: this.state.vpc,
      internetFacing: true,
      securityGroup: this.state.externalSecurityGroup,
    });
  ...
  createColorTeller(...colors: string[]) {
    let create = (color: string, serviceName: string) => {
      ...
      new FargateService(this, `ColorTellerService-${color}`, {
        cluster: this.state.cluster,
  ...
  createMesh() {
    this.state.mesh = new CfnMesh(this, "Mesh", {
  ...
  createVirtualNodes() {
    let create = (name: string, serviceName?: string) => {
      ...
      (new CfnVirtualNode(this, nodeName, {
        meshName: this.state.mesh.meshName,
  ...
  createVirtualRouter(): CfnVirtualRouter {
    ...
    router.addDependsOn(this.state.mesh);
    return router;
  }
